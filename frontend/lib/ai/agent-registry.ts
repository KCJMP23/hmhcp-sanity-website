...
// Internal mutable types for registry operations
interface MutableAgentInstance {
  id: string;
...
export class HealthcareAgentRegistry extends EventEmitter {
  private readonly agents = new Map<string, MutableAgentInstance>();
  private readonly agentConfigs = new Map<string, AgentConfiguration>();
   ...
   */
  private toReadonlyAgentInstance(agent: MutableAgentInstance): AgentInstance {
    return {
      id: agent.id,
      config: agent.config,
      status: agent.status,
   ...
   */
  private initializeMutableAgentMetrics(): MutableAgentMetrics {
    return {
      tasksCompleted: 0,
      tasksFailed: 0,
   ...
   */
  async registerAgent(config: AgentConfiguration): Promise<void> {
    try {
      ...
      const instance: MutableAgentInstance = {
        id: config.id,
        config,
        status: 'idle',
   ...
   */
  async findBestAgentForTask(task: WorkflowTask): Promise<AgentInstance | null> {
  ...
  async updateAgentStatus(agentId: string, status: AgentStatus): Promise<void> {
    ...
    if (previousStatus !== status) {
      this.emit('agent-status-changed', {
        ...
        type: 'agent-status-changed' as any,
        source: 'agent-registry',
        timestamp: new Date(),
   ...
   */
  async assignTaskToAgent(agentId: string, task: WorkflowTask): Promise<void> {
    const agent = this.agents.get(agentId);
    if (!agent) {
      throw new Error(`Agent ${agentId} not found`);
    }
   ...
   */
  async completeTaskForAgent(agentId: string, taskId: string, success: boolean): Promise<void> {
    const agent = this.agents.get(agentId);
    if (!agent) {
      throw new Error(`Agent ${agentId} not found`);
    }
  ...
  private async validateAgentConfig(config: AgentConfiguration): Promise<void> {
  ...
  private findSuitableAgents(task: WorkflowTask): MutableAgentInstance[] {
    return Array.from(this.agents.values())
      .filter(agent => {
        ...
        const hasRequiredCapabilities = task.requiredCapabilities.every(requiredCap =>
          agent.config.capabilities.some(agentCap => agentCap.name === requiredCap)
        );
  ...
  private calculateLoadFactor(agent: MutableAgentInstance): number {
    const activeTasks = agent.currentTask ? 1 : 0;
    return activeTasks / agent.config.maxConcurrentTasks;
  }

  private calculateHealthScore(agent: MutableAgentInstance): number {
    let score = 1.0;
  ...
  private handleDistributedRegistryUpdate(update: any): void {
  ...
  private async storeAgentInRedis(agent: AgentInstance): Promise<void> {
  ...
  private async removeAgentFromRedis(agentId: string): Promise<void> {
...
class AgentLoadBalancer {
  selectBestAgent(agents: MutableAgentInstance[], task: WorkflowTask): MutableAgentInstance | null {
    if (agents.length === 0) {
      return null;
    }
  ...
  private calculateAgentScore(agent: MutableAgentInstance, task: WorkflowTask): number {
    let score = 0;
  ...
  private isHealthcareTask(task: WorkflowTask): boolean {
    const healthcareTaskTypes: TaskType[] = [
  ...
  private calculateCapabilityMatch(agent: MutableAgentInstance, task: WorkflowTask): number {
